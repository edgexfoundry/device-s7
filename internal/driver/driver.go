...
outloop:
	for j := 0; j <= times; j++ {
		...
		for i, req := range tmp_reqs {
			...
			if err != nil {
				count++
				s.lc.Errorf("transfer nodeName to dbInfo failed,err =%v", err)
...
func (s *Driver) HandleWriteCommands(deviceName string, protocols map[string]models.ProtocolProperties, reqs []sdkModel.CommandRequest,
	...
	for i, req := range reqs {
		...
		if err != nil {
			count++
			s.lc.Errorf("transfer nodeName %v to dbInfo failed,err =%v", nodeName, err)
			dbInfo = &DBInfo{
				Area:       0,
				DBNumber:   0,
				Start:      0,
...
func (s *Driver) getDBInfo(variable string) (dbInfo *DBInfo, err error) {
	...
	case "DB": //Data Block
		...
		if len(dbArray) < 2 {
			...
			return nil, fmt.Errorf("DB variable %+v is invaliad", variable)
		}
		dbNo, err = strconv.ParseInt(string(string(dbArray[0])[2:]), 10, 16)
		if err != nil {
			s.lc.Errorf("The point address of %+v parse dbNo to int failed.err:%v", variable, err)
			return nil, fmt.Errorf("the point address of %+v parese dbNo to int failed.err:%v", variable, err)
		}
		dbIndex, err = strconv.ParseInt(string(string(dbArray[1])[3:]), 10, 16)
		if err != nil {
			s.lc.Errorf("The point address of %+v parse dbIndex to int failed.err:%v", variable, err)
			return nil, fmt.Errorf("the point address of %+v parese dbIndex to int failed.err:%v", variable, err)
		}
		...
		case "DBX": //bit
			...
			if err != nil {
				s.lc.Errorf("The point address of %+v parse dbBit to int failed.err:%v", variable, err)
				return nil, fmt.Errorf("the point address of %+v parse dbBit to int failed.err:%v", variable, err)
			}
	...
	if err != nil {
		s.lc.Errorf("The point address of %+v parse to int failed.err:%v", variable, err)
		return nil, fmt.Errorf("the point address of %+v parese to int failed.err:%v", variable, err)
	}
	return &DBInfo{
		Area:       area,
		DBNumber:   int(dbNo),
